...
class Bind(Node):
    def __init__(self, name, expr, file, line, col, traced, show):
        super().__init__(file, line, col)
        self.name = name
        self.expr = expr
        self.traced = traced
...
class Fn(Node):
    def __init__(self, params, body, file, line, col):
        super().__init__(file, line, col)
        self.params = params
        self.body = body
...
class List(Node):
    def __init__(self, elems, file, line, col):
        super().__init__(file, line, col)
        self.elems = elems
...
class Parser:
    ...
    def parse_bind(self):
        ...
        while self.peek()[1] == '!':
            show += 1
            self.advance()
        return Bind(name, expr, self.filepath, tok[2], tok[3], traced, show)
    ...
    def parse_list(self):
        ...
        if self.peek()[1] != ']':
            ...
            while self.peek()[1] == ',':
                ...
                elems.append(self.parse_expr())
        self.expect('PUNCT', ']')
        return List(elems, self.filepath, tok[2], tok[3])
