"""Phase1P proof — pure sub-theorems, no shared state.

phase1_base(): ∀-closed theorem proving the base case
phase1_step_case(): ∀-closed theorem proving the step case
phase1(): omega induction, instantiates both, produces Phase1P
"""
...
def _P_components(d, q0, tape, c0):
    """Return (base_cond, dom_bound, step_valid, P) builders for given params.
    All bound vars are fresh per call."""
    ...
    def base_cond(tr):
        return Forall(zp, Implies(Empty(zp), Apply(tr, zp, c0)))
    def dom_bound(tr, nn):
        return Forall(xd, Forall(yd, Implies(Apply(tr, xd, yd), Or(In(xd, nn), Eq(xd, nn)))))
    def step_valid(tr, nn):
        return Forall(k, Implies(In(k, nn), Forall(sk, Implies(Successor(sk, k),
            Forall(ck, Implies(Apply(tr, k, ck),
                Exists(ck1, And(Apply(tr, sk, ck1), TMStep(d, ck, ck1)))))))))
    def P(nn):
        return Exists(trace, Exists(cn, And(FuncDef(trace), And(dom_bound(trace, nn),
            And(base_cond(trace), And(TMConfig(cn, q0, nn, tape),
            And(Apply(trace, nn, cn), step_valid(trace, nn))))))))
    return base_cond, dom_bound, step_valid, P, trace, cn
...
def phase1_base():
    ...
    where P(z) = ∃trace,cn. Function∧dom_bound∧base_cond∧TMConfig∧Apply∧step_valid.
    Left: [Pairing]"""
    ...
    base_cond,dom_bound,step_valid,P,trace,cn = _P_components(d,q0,tape,c0)
    zp=Var(postfix='zp');xd=Var(postfix='xd');yd=Var(postfix='yd')
    ...
    sv_inner=Forall(sk,Implies(Successor(sk,k),Forall(ck,Implies(Apply(t_sing,k,ck),Exists(ck1,And(Apply(t_sing,sk,ck1),TMStep(d,ck,ck1)))))))
    nkz=Not(In(k,z));gb=Proof(Sequent([In(k,z),nkz],[]),'not_left',[ax(In(k,z))],principal=nkz)
    gb=Proof(Sequent(gb.sequent.left,[sv_inner]),'weakening_right',[gb],principal=sv_inner)
    gb=cut(gb,nkz,got_nkz)
    imp_sv=Implies(In(k,z),sv_inner);lsv=[f for f in gb.sequent.left if not same(f,In(k,z))]
    got_sv=Proof(Sequent(lsv,[imp_sv]),'implies_right',[gb],principal=imp_sv)
    got_sv=Proof(Sequent(got_sv.sequent.left,[Forall(k,imp_sv)]),'forall_right',[got_sv],principal=Forall(k,imp_sv),term=k)
    # Assemble + eir
    pza=_mk_and(got_app_s,got_sv);pza=_mk_and(ax(cfg_c0),pza);pza=_mk_and(got_bc,pza);pza=_mk_and(got_db,pza);pza=_mk_and(got_func_s,pza)
    bfc=And(FuncDef(t_sing),And(dom_bound(t_sing,z),And(base_cond(t_sing),And(TMConfig(cn,q0,z,tape),And(Apply(t_sing,z,cn),step_valid(t_sing,z))))))
