...
def exists_unique_bridge(phi, pred, x, vars):
    ...
    phi: formula with x free (characterizing property).
    pred: In(x, something) with x free.
    ...
    u = Var()
    phi_u = _subst(phi, x, u); eu = ExistsUnique(u, phi_u)
    ...
    gi = apply_thm(eq_substitution(), [x, w, pred.right])
    ...
    le = [f for f in gr.sequent.left if not same(f, eu)]
    gr = Proof(Sequent(le, [imp_eu]), 'implies_right', [gr], principal=imp_eu)
    for v in vars:
