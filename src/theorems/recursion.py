...
if not hasattr(ExistsUnique, 'eq'):
    ExistsUnique.eq = lambda self, other, env, expand, eq: (
        isinstance(other, ExistsUnique) and
        eq(self.body, other.body, env + [(self.var, other.var)], expand))
...
def _ordpair_set_transfer(z_var, p_var, x_var, y_var, eq_zp_proof, ordp_proof):
    """Given eq_zp_proof: [ctx1] |- Eq(z, p) and ordp_proof: [ctx2] |- OrdPair(p, x, y),
    derive [ctx1 + ctx2] |- OrdPair(z, x, y).
    OrdPair(t,a,b) = Forall(sa, Implies(Sing(sa,a), Forall(pab, Implies(PS(pab,a,b), PS(t,sa,pab))))).
    From OrdPair(p,...): instantiate sa,pab and MP to get PS(p,sa,pab).
    ...
    ps_pab_xy = PairSet(pab, x_var, y_var)
    ps_p_sa_pab = PairSet(p_var, sa, pab)
    ps_z_sa_pab = PairSet(z_var, sa, pab)
    ordp_p = OrdPair(p_var, x_var, y_var)

    # From OrdPair(p, x, y): instantiate sa, MP Sing, instantiate pab, MP PairSet -> PS(p, sa, pab)
    ordp_body_sa = Implies(sing_sa_x, Forall(pab, Implies(ps_pab_xy, ps_p_sa_pab)))
    got_ordp_sa = fl(ordp_p, ordp_body_sa, sa)
    got_ordp_pab = mp(got_ordp_sa, ax(sing_sa_x), sing_sa_x,
        Forall(pab, Implies(ps_pab_xy, ps_p_sa_pab)))
    got_ordp_ps = apply_thm(got_ordp_pab, [pab], ps_pab_xy, ps_p_sa_pab, ax(ps_pab_xy))
    # [ordp_p, Sing(sa,x), PS(pab,x,y)] |- PS(p, sa, pab)

    # From Eq(z, p): char_transfer PairSet(p,...) to PairSet(z,...)
    iff_zp = Iff(In(wv, z_var), In(wv, p_var))
    or_eq = Or(Eq(wv, sa), Eq(wv, pab))
    iff_in_p = Iff(In(wv, p_var), or_eq)
    iff_in_z = Iff(In(wv, z_var), or_eq)
    fl_eq = fl(Eq(z_var, p_var), iff_zp, wv)
    ct = char_transfer(In(wv, z_var), In(wv, p_var), or_eq, [])
    got_ps_z_w = mp(mp(ct, fl_eq, iff_zp, Implies(iff_in_p, iff_in_z)),
        fl(ps_p_sa_pab, iff_in_p, wv), iff_in_p, iff_in_z)
    # [Eq(z,p), PS(p,sa,pab)] |- Iff(In(wv,z), Or(...))
    ...
    # [Eq(z,p), PS(p,sa,pab)] |- PS(z, sa, pab)

    # Cut PS(p,sa,pab):
    got_ps_z_full = Proof(Sequent(
        list(got_ordp_ps.sequent.left) + [Eq(z_var, p_var)], [fa_ps_z]), 'cut',
        [wr(wl(got_ordp_ps, Eq(z_var, p_var)), fa_ps_z),
         wl(got_ps_z, *got_ordp_ps.sequent.left)], principal=ps_p_sa_pab)
    # [ordp_p, Sing(sa,x), PS(pab,x,y), Eq(z,p)] |- PS(z, sa, pab)

    # Close into OrdPair(z, x, y)
    imp_ps = Implies(ps_pab_xy, ps_z_sa_pab)
    rem_ps = [f_ for f_ in got_ps_z_full.sequent.left if not same(f_, ps_pab_xy)]
    got_imp_ps = Proof(Sequent(rem_ps, [imp_ps]), 'implies_right', [got_ps_z_full], principal=imp_ps)
    fa_pab = Forall(pab, imp_ps)
    got_fa_pab = Proof(Sequent(rem_ps, [fa_pab]), 'forall_right', [got_imp_ps], principal=fa_pab, term=pab)
    imp_sing = Implies(sing_sa_x, fa_pab)
    rem_sing = [f_ for f_ in got_fa_pab.sequent.left if not same(f_, sing_sa_x)]
    got_imp_sing = Proof(Sequent(rem_sing, [imp_sing]), 'implies_right', [got_fa_pab], principal=imp_sing)
    ordp_z = OrdPair(z_var, x_var, y_var)
    got_ordp_z = Proof(Sequent(rem_sing, [ordp_z]), 'forall_right', [got_imp_sing], principal=ordp_z, term=sa)
    # [ordp_p, Eq(z,p)] |- OrdPair(z, x, y)

    # Cut with provided proofs
    from tactics import cut
    got = cut(got_ordp_z, ordp_p, ordp_proof)
    got = cut(got, Eq(z_var, p_var), eq_zp_proof)
    return got
...
def eq_apply_transfer():
    ...
    # === Step 1: Unpack Apply(v,x1,y) -> OrdPair(p,x1,y) + In(p,v) ===
    ordp_x1 = OrdPair(p, x1, y)
    in_pv = In(p, v)
    and_ordp_in = And(ordp_x1, in_pv)

    got_ordp = apply_thm(and_elim_left(ordp_x1, in_pv, []), [], and_ordp_in, ordp_x1, ax(and_ordp_in))
    got_inf = apply_thm(and_elim_right(ordp_x1, in_pv, []), [], and_ordp_in, in_pv,
        Proof(Sequent([and_ordp_in], [and_ordp_in]), 'axiom', principal=and_ordp_in))

    # === Step 2: ordpair_eq_transfer: Eq(x1,x2) -> Eq(y,y) -> OrdPair(p,x1,y) -> OrdPair(p,x2,y) ===
    oet = ordpair_eq_transfer()
    er = eq_reflexive()
    ordp_x2 = OrdPair(p, x2, y)
    got_eq_refl = apply_thm(er, [y], concl=Eq(y, y))
    got_ordp_x2 = apply_thm(oet, [x1, y, x2, y, p], eq_x,
        Implies(Eq(y, y), Implies(ordp_x1, ordp_x2)), ax(eq_x))
    got_ordp_x2 = mp(got_ordp_x2, got_eq_refl, Eq(y, y), Implies(ordp_x1, ordp_x2))
    got_ordp_x2 = mp(got_ordp_x2, got_ordp, ordp_x1, ordp_x2)
    # got_ordp_x2: [eq_x, and_ordp_in] |- OrdPair(p, x2, y)

    # === Step 3: Build And(OrdPair(p,x2,y), In(p,v)) -> eir -> Apply(v,x2,y) ===
    and_ordp_x2_in = And(ordp_x2, in_pv)
    ai = and_intro(ordp_x2, in_pv, [])
    got_and_x2 = mp(apply_thm(ai, [], ordp_x2, Implies(in_pv, and_ordp_x2_in), got_ordp_x2),
        got_inf, in_pv, and_ordp_x2_in)
    got_app2 = eir(got_and_x2, And(OrdPair(qv, x2, y), In(qv, v)), qv, p)
    # got_app2: [eq_x, and_ordp_in] |- Apply(v, x2, y)

    # === Step 4: eel p from and_ordp_in, connect to Apply(v,x1,y) ===
    got_app_from_and = eel(got_app2, and_ordp_in, p)
    # got_app_from_and: [eq_x, Apply(v,x1,y)] |- Apply(v, x2, y)

    # === Close: discharge and forall ===
    app1_on_left = [f_ for f_ in got_app_from_and.sequent.left if same(f_, app1)][0]
    proof = got_app_from_and
    for h in [app1, eq_x]:
        imp_h = Implies(h, proof.sequent.right[0])
        remaining = [f_ for f_ in proof.sequent.left if not same(f_, h)]
        proof = Proof(Sequent(remaining, [imp_h]), 'implies_right', [proof], principal=imp_h)
...
def eq_apply_val_transfer():
    ...
    # === Step 1: Unpack Apply(v,x,y1) -> OrdPair(p,x,y1) + In(p,v) ===
    ordp_y1 = OrdPair(p, x, y1)
    in_pv = In(p, v)
    and_ordp_in = And(ordp_y1, in_pv)

    got_ordp = apply_thm(and_elim_left(ordp_y1, in_pv, []), [], and_ordp_in, ordp_y1, ax(and_ordp_in))
    got_inf = apply_thm(and_elim_right(ordp_y1, in_pv, []), [], and_ordp_in, in_pv,
        Proof(Sequent([and_ordp_in], [and_ordp_in]), 'axiom', principal=and_ordp_in))

    # === Step 2: ordpair_val_transfer: Eq(y1,y2) -> OrdPair(p,x,y1) -> OrdPair(p,x,y2) ===
    ovt = ordpair_val_transfer()
    ordp_y2 = OrdPair(p, x, y2)
    got_ordp_y2 = apply_thm(ovt, [p, x, y1, y2], eq_y,
        Implies(ordp_y1, ordp_y2), ax(eq_y))
    got_ordp_y2 = mp(got_ordp_y2, got_ordp, ordp_y1, ordp_y2)
    # got_ordp_y2: [eq_y, and_ordp_in] |- OrdPair(p, x, y2)

    # === Step 3: Build And(OrdPair(p,x,y2), In(p,v)) -> eir -> Apply(v,x,y2) ===
    and_ordp_y2_in = And(ordp_y2, in_pv)
    ai = and_intro(ordp_y2, in_pv, [])
    got_and_y2 = mp(apply_thm(ai, [], ordp_y2, Implies(in_pv, and_ordp_y2_in), got_ordp_y2),
        got_inf, in_pv, and_ordp_y2_in)
    got_app2 = eir(got_and_y2, And(OrdPair(qv, x, y2), In(qv, v)), qv, p)
    # got_app2: [eq_y, and_ordp_in] |- Apply(v, x, y2)

    # === Step 4: eel p from and_ordp_in, connect to Apply(v,x,y1) ===
    got_app_from_and = eel(got_app2, and_ordp_in, p)
    # got_app_from_and: [eq_y, Apply(v,x,y1)] |- Apply(v, x, y2)
...
def extend_function():
    ...
    got_ordz2 = _ordpair_set_transfer(zr, p, x0, y0, got_eq, ax(ordp))
...
def singleton_is_recapprox():
    ...
    # === Derive OrdPair(zz, ev, a) from OrdPair(p, ev, a) + Eq(zz, p) ===
    # OrdPair(t, x, y) = Forall(sa, Implies(Sing(sa,x), Forall(pab, Implies(PS(pab,x,y), PS(t,sa,pab)))))
    # From OrdPair(p, ev, a): instantiate sa=sa2, get Sing(sa2,ev)->Forall(pab, ...).
    # MP: Forall(pab, Implies(PS(pab,ev,a), PS(p,sa2,pab))). Instantiate pab=pab2.
    # MP: PS(pab2,ev,a) -> PS(p, sa2, pab2).
    # From Eq(zz,p): char_transfer on PairSet to get PS(zz, sa2, pab2) from PS(p, sa2, pab2).
    # Close: implies_right PS(pab2,ev,a), forall_right pab2, implies_right Sing(sa2,ev), forall_right sa2.

    sing_sa2_e = Singleton(sa2, ev)
    ps_pab2_ea = PairSet(pab2, ev, a)
    ps_p_sa_pab = PairSet(p, sa2, pab2)
    ps_z_sa_pab = PairSet(zz, sa2, pab2)

    # From OrdPair(p, ev, a): instantiate and MP to get PS(p, sa2, pab2)
    ordp_exp = ordp.expand()  # Forall(sa_i, Implies(Sing(sa_i, ev), Forall(pab_i, Implies(PS(pab_i, ev, a), PS(p, sa_i, pab_i)))))
    # Use fl to instantiate sa_i with sa2:
    ordp_body_sa2 = Implies(sing_sa2_e, Forall(pab2, Implies(ps_pab2_ea, ps_p_sa_pab)))
    got_ordp_sa = fl(ordp, ordp_body_sa2, sa2)
    # [ordp] |- Implies(Sing(sa2,ev), Forall(pab2, Implies(PS(pab2,ev,a), PS(p,sa2,pab2))))
    got_ordp_pab = mp(got_ordp_sa, ax(sing_sa2_e), sing_sa2_e,
        Forall(pab2, Implies(ps_pab2_ea, ps_p_sa_pab)))
    got_ordp_ps = apply_thm(got_ordp_pab, [pab2], ps_pab2_ea, ps_p_sa_pab, ax(ps_pab2_ea))
    # [ordp, Sing(sa2,ev), PS(pab2,ev,a)] |- PS(p, sa2, pab2)

    # From Eq(zz,p) + PS(p, sa2, pab2): derive PS(zz, sa2, pab2) via char_transfer
    # Eq(zz,p) gives In(w,zz) iff In(w,p). PairSet(p,...) = Forall(w, Iff(In(w,p), Or(...))).
    # char_transfer: Iff(In(w,zz), In(w,p)) + Iff(In(w,p), Or(...)) -> Iff(In(w,zz), Or(...)).
    iff_zp = Iff(In(wv, zz), In(wv, p))
    or_eq = Or(Eq(wv, sa2), Eq(wv, pab2))
    iff_in_p = Iff(In(wv, p), or_eq)
    iff_in_z = Iff(In(wv, zz), or_eq)
    ...
    got_ps_z_w = mp(mp(ct_ps, fl_eq_zp, iff_zp, Implies(iff_in_p, iff_in_z)),
        fl(ps_p_sa_pab, iff_in_p, wv), iff_in_p, iff_in_z)
    # [Eq(zz,p), PS(p,sa2,pab2)] |- Iff(In(wv,zz), Or(...))
    fa_ps_z = Forall(wv, iff_in_z)
    got_ps_z = Proof(Sequent(got_ps_z_w.sequent.left, [fa_ps_z]), 'forall_right',
        [got_ps_z_w], principal=fa_ps_z, term=wv)
    # [Eq(zz,p), PS(p,sa2,pab2)] |- PairSet(zz, sa2, pab2)

    # Cut PS(p,sa2,pab2) from got_ordp_ps into got_ps_z:
    got_ps_z_full = Proof(Sequent(
        [f_ for f_ in got_ordp_ps.sequent.left] + [Eq(zz, p)], [fa_ps_z]), 'cut',
        [wr(wl(got_ordp_ps, Eq(zz, p)), fa_ps_z),
         wl(got_ps_z, *got_ordp_ps.sequent.left)], principal=ps_p_sa_pab)
    # [ordp, Sing(sa2,ev), PS(pab2,ev,a), Eq(zz,p)] |- PairSet(zz, sa2, pab2)

    # Close into OrdPair(zz, ev, a) = Forall(sa, Implies(Sing, Forall(pab, Implies(PS, PS_z))))
    # implies_right PS(pab2,ev,a):
    imp_ps = Implies(ps_pab2_ea, ps_z_sa_pab)
    rem_ps = [f_ for f_ in got_ps_z_full.sequent.left if not same(f_, ps_pab2_ea)]
    got_imp_ps = Proof(Sequent(rem_ps, [imp_ps]), 'implies_right', [got_ps_z_full], principal=imp_ps)
    # forall_right pab2 (pab2 not free in ordp, Sing(sa2,ev), Eq(zz,p)):
    fa_pab = Forall(pab2, imp_ps)
    got_fa_pab = Proof(Sequent(rem_ps, [fa_pab]), 'forall_right', [got_imp_ps], principal=fa_pab, term=pab2)
    # implies_right Sing(sa2,ev):
    imp_sing = Implies(sing_sa2_e, fa_pab)
    rem_sing = [f_ for f_ in got_fa_pab.sequent.left if not same(f_, sing_sa2_e)]
    got_imp_sing = Proof(Sequent(rem_sing, [imp_sing]), 'implies_right', [got_fa_pab], principal=imp_sing)
    # forall_right sa2 (sa2 not free in ordp, Eq(zz,p)):
    ordp_zz = OrdPair(zz, ev, a)
    got_ordp_zz = Proof(Sequent(rem_sing, [ordp_zz]), 'forall_right', [got_imp_sing], principal=ordp_zz, term=sa2)
    # [ordp, Eq(zz,p)] |- OrdPair(zz, ev, a)

    # Existential intro x=ev, y=a for the Relation conclusion
    x_rel, y_rel = Var(), Var()
    got_ex_y = eir(got_ordp_zz, OrdPair(zz, ev, y_rel), y_rel, a)
    got_ex_xy = eir(got_ex_y, Exists(y_rel, OrdPair(zz, x_rel, y_rel)), x_rel, ev)
    # got_ex_xy: [ordp, Eq(zz,p)] |- exists x,y. OrdPair(zz, x, y)
...
def rec_approx_val_in_w():
    """RecApprox values stay in w (omega induction, no Replacement).
    Ext, Inf, Sep, Pairing, Union |- forall v, a, f, w, n, y.
        RecApprox(v,a,f,w) -> In(a,w) -> Omega(w) ->
        (forall x. In(x,w) -> forall z. Apply(f,x,z) -> In(z,w)) ->
        In(n,w) -> Apply(v,n,y) -> In(y,w)

    Omega induction: P(n) = ∀y. Apply(v,n,y) → In(y,w)
    Base: rec_approx_zero gives v(0)=a, In(a,w) gives In(y,w).
    Step: RecApprox step + Function(v) + func_unique give v(S(n))=f(v(n)),
          IH gives v(n)∈w, range_closed gives f(v(n))∈w."""
    ...
    from theorems.logic import (iff_intro, iff_mp, iff_mp_rev, and_intro,
        and_elim_left, and_elim_right, eq_substitution)
    ...
    from vocab import RecApprox, Apply, Empty, Omega, Successor, Inductive, Subset
    import core.zfc as zfc

    v, a, f, w = Var(postfix='v'), Var(postfix='a'), Var(postfix='f'), Var(postfix='w')
    n, y, b, c = Var(postfix='n'), Var(postfix='y'), Var(postfix='b'), Var(postfix='c')
    xr, zr = Var(postfix='xr'), Var(postfix='zr')
    sn = Var(postfix='sn')
    pv = Var(postfix='pv')  # the inductive set
    xv = Var(postfix='xv')  # quantified variable for separation
    ...
    got_ex_pv = apply_thm(sep_ax, [w, v, w], concl=Exists(pv, char_pv))
    # [Sep] |- ∃pv. char_pv

    # Helper: char_pv forward and backward
    inst_iff = Iff(In(xv, pv), And(In(xv, w), P(xv)))

    def char_fwd(term):
        """[char_pv, In(term,pv)] |- In(term,w) ∧ P(term)"""
    ...
    def char_bwd(term, got_in_w, got_P):
        """[got_in_w.left, got_P.left, char_pv] |- In(term,pv)"""
        iff_inst = Iff(In(term, pv), And(In(term, w), P(term)))
        got_iff = fl(char_pv, iff_inst, term)
    ...
    # === Base case: Empty(zero) → In(zero, pv) ===
    ...
    # [omega_w, Empty(zero), Ext, Inf] |- In(zero, w)

    # P(zero): ∀y. Apply(v,zero,y) → In(y,w)
    # From rec_approx_zero: RecApprox(v,a,f,w) → Empty(zero) → Apply(v,zero,y) → Eq(y,a)
    raz = rec_approx_zero()
    got_eq = apply_thm(raz, [v, a, f, w, zero, y], ra,
        Implies(empty_zero, Implies(Apply(v, zero, y), Eq(y, a))), ax(ra))
    got_eq = mp(got_eq, ax(empty_zero), empty_zero, Implies(Apply(v, zero, y), Eq(y, a)))
    got_eq = mp(got_eq, ax(Apply(v, zero, y)), Apply(v, zero, y), Eq(y, a))
    # [ra, Empty(zero), Apply(v,zero,y)] |- Eq(y,a)

    # Eq(y,a) + In(a,w) → In(y,w) via eq_substitution
    esub = eq_substitution()
    got_iff_yw = apply_thm(esub, [y, a, w], Eq(y, a), Iff(In(y, w), In(a, w)), got_eq)
    got_in_yw = mp(apply_thm(iff_mp_rev(In(y, w), In(a, w), []),
        [], Iff(In(y, w), In(a, w)), Implies(In(a, w), In(y, w)), got_iff_yw),
        ax(in_a_w), In(a, w), In(y, w))
    # [ra, Empty(zero), Apply(v,zero,y), in_a_w, Ext] |- In(y,w)

    # Close to get P(zero):
    imp_app = Implies(Apply(v, zero, y), In(y, w))
    left_imp = [f_ for f_ in got_in_yw.sequent.left if not same(f_, Apply(v, zero, y))]
    got_P_zero = Proof(Sequent(left_imp, [imp_app]), 'implies_right', [got_in_yw], principal=imp_app)
    p_zero = Forall(y, imp_app)
    got_P_zero = Proof(Sequent(got_P_zero.sequent.left, [p_zero]),
        'forall_right', [got_P_zero], principal=p_zero, term=y)
    # [ra, Empty(zero), in_a_w, Ext] |- P(zero)

    # In(zero, pv) via char_bwd:
    got_base = char_bwd(zero, got_zero_in_w, got_P_zero)
    # [ra, Empty(zero), in_a_w, omega_w, char_pv, Ext, Inf] |- In(zero, pv)

    # Discharge Empty(zero), close ∀zero: Inductive base part
    ...
    # [...] |- ∀zero. Empty(zero) → In(zero, pv)

    # === Step case: In(n,pv) → Successor(sn,n) → In(sn,pv) ===
    # From In(n,pv): get In(n,w) ∧ P(n) via char_fwd
    got_and_n = char_fwd(n)
    # [char_pv, In(n,pv)] |- And(In(n,w), P(n))
    ...
    # [char_pv, In(n,pv), omega_w, Succ(sn,n), Ext, Inf] |- In(sn,w)

    # P(sn): ∀y. Apply(v,sn,y) → In(y,w)
    # Need: from Apply(v,sn,y) derive In(y,w).
    # Strategy:
    #   1. From RecApprox prop 5 + In(n,w) + Succ(sn,n) + ∃d.Apply(v,sn,d):
    #      get (∃b.Apply(v,n,b)) ∧ (∀b.Apply(v,n,b)→∀c.Apply(f,b,c)→Apply(v,sn,c))
    #   2. Let b be witness of Apply(v,n,b).
    #   3. From P(n): In(b,w).
    #   4. From RecApprox prop 3: Apply(v,n,b)→∃c.Apply(f,b,c). Let c be witness.
    #   5. From step 1 second part: Apply(v,n,b)→Apply(f,b,c)→Apply(v,sn,c).
    #   6. So Apply(v,sn,c). From Function(v) + Apply(v,sn,y): y=c (func_unique_thm).
    #   7. Apply(f,b,c) with c=y: Apply(f,b,y).
    #   8. range_closed + In(b,w) + Apply(f,b,y) → In(y,w). ✓
    #
    # This requires extracting from RecApprox. RecApprox is a conjunction.
    # Let me use apply_thm on ax(ra) to instantiate the step property.

    # RecApprox step (prop 5): instantiate n and sn from the expansion
    # RecApprox(v,a,f,w) has (among other things):
    #   Forall(n', Implies(In(n',w), Forall(s', Implies(Succ(s',n'),
    #     Implies(Exists(d, Apply(v,s',d)),
    #       And(Exists(b', Apply(v,n',b')),
    #           Forall(b', Implies(Apply(v,n',b'), Forall(c', Implies(Apply(f,b',c'), Apply(v,s',c')))))))))))

    # Instead of extracting from the expansion directly (very complex),
    # let me use the EXISTING rec_exists_step lemma or build from primitives.
    # Actually, let me directly use fl on ra for the step property.

    # The step property is the 5th conjunct of RecApprox. After expanding and_elim_right
    # 4 times, I get it. But that's tedious. Let me instead use a simpler approach:
    # ASSUME the needed facts and prove In(y,w), then discharge at the end.

    # For the step case, assume:
    # - Apply(v,sn,y) [what we're showing In(y,w) for]
    # - Apply(v,n,b) [v is defined at n — from RecApprox step property]
    # - Apply(f,b,y) [v(sn)=f(v(n)) — derived from Function(v) + RecApprox step]
    # Then: range_closed + In(b,w) → In(y,w)

    # From P(n) + Apply(v,n,b): In(b,w)
    app_vnb = Apply(v, n, b)
    got_b_in_w = apply_thm(got_P_n, [b], app_vnb, In(b, w), ax(app_vnb))
    # [char_pv, In(n,pv), Apply(v,n,b)] |- In(b,w)

    # From range_closed + In(b,w) + Apply(f,b,y): In(y,w)
    app_fby = Apply(f, b, y)
    got_rc = apply_thm(ax(range_closed), [b], In(b, w),
        Forall(zr, Implies(Apply(f, b, zr), In(zr, w))), got_b_in_w)
    got_in_y = apply_thm(got_rc, [y], app_fby, In(y, w), ax(app_fby))
    # [char_pv, In(n,pv), Apply(v,n,b), range_closed, Apply(f,b,y)] |- In(y,w)

    # Now I need to get Apply(v,n,b) and Apply(f,b,y) from RecApprox + Apply(v,sn,y).
    # This is the complex part. Let me use func_unique_thm for the key step.
    #
    # From RecApprox step: Apply(v,n,b)→Apply(f,b,c)→Apply(v,sn,c)
    # From Apply(v,sn,y) + Apply(v,sn,c) + Function(v): y=c (func_unique)
    # So Apply(f,b,y) (since c=y).
    #
    # For now, let me take Apply(v,n,b) and Apply(f,b,y) as hypotheses
    # and discharge them later via existential elimination from RecApprox properties.
    # This keeps the proof cleaner.

    # Close P(sn): ∀y. Apply(v,sn,y) → In(y,w)
    # Currently have: [..., Apply(v,n,b), Apply(f,b,y)] |- In(y,w)
    # Need to eliminate b (it's existential from RecApprox step)
    # and derive Apply(f,b,y) from the structure.
    #
    # Simplification: instead of fully extracting from RecApprox, use
    # the existing rec_exists_step or a direct and_elim chain.
    # For MINIMAL working version: add these as hypotheses and complete later.
    #
    # ACTUALLY: let me just close directly. The key insight:
    # Given P(n)=[∀y.Apply(v,n,y)→In(y,w)] and Apply(v,n,b), we get In(b,w).
    # Given range_closed and In(b,w) and Apply(f,b,y), we get In(y,w).
    # These two facts (Apply(v,n,b) and Apply(f,b,y)) come from the assumption
    # that v(sn)=f(v(n)) which IS RecApprox's step property.
    # For the formal proof, they need to be on the left.
    ...
    # Cut ex_b from got_in_y2's left with its DERIVATION from RecApprox:
    # got_in_y2 has ex_b on left. got_exb_derived proves ex_b on right.
    # After cut: ex_b removed from left, derivation's context added.

    # DERIVATION: from Apply(v,sn,y) + RecApprox + In(n,w) + Succ(sn,n), derive ex_b.
    # RecApprox prop 5 gives the step structure. Extract it:
    ...
    # Also Apply(v,sn,c) might be on left with c free. Cut it with got_vsn_c derived from app_fbc:
    app_vsn_c = Apply(v, sn, c)
    if any(same(app_vsn_c, f_) for f_ in got_exb_derived.sequent.left):
        # app_vsn_c was derived from app_vnb + app_fbc + step. It's redundant. Just cut with its derivation.
        got_exb_derived = cut(got_exb_derived, app_vsn_c, got_vsn_c)
    ...
    # === Inductive(pv): combine base + step ===
    ind_pv = Inductive(pv)
    # Inductive = base ∧ step (Empty→In and succ-closed)
    # Package got_ind_base and got_step into Inductive(pv)
    # Inductive(pv) = ∀e. Empty(e)→In(e,pv) ∧ ∀x∈pv.∀s. Succ(s,x)→In(s,pv)
    # This matches got_ind_base and got_step.

    # For now: combine contexts and build And
    all_base = list(got_ind_base.sequent.left)
    all_step = list(got_step.sequent.left)
    all_ctx = list(all_base)
    for f_ in all_step:
    ...
    # Build And(base, step) = Inductive representation
    # Actually Inductive is a definition. Let me just use it directly.
    # Inductive(pv) = And(∀e.Empty(e)→In(e,pv), ∀x∈pv.∀s.Succ(s,x)→In(s,pv))
    ind_base_formula = got_ind_base_w.sequent.right[0]
    ind_step_formula = got_step_w.sequent.right[0]
    and_ind = And(ind_base_formula, ind_step_formula)
    got_ind = mp(apply_thm(and_intro(ind_base_formula, ind_step_formula, []),
        [], ind_base_formula, Implies(ind_step_formula, and_ind), got_ind_base_w),
        got_step_w, ind_step_formula, and_ind)
    # all_ctx |- And(base, step) = Inductive(pv)

    # === Subset(pv, w) ===
    # From char_fwd: In(x,pv) → In(x,w) (first conjunct of char)
    xs = Var()
    got_fwd_xs = char_fwd(xs)
    got_xs_in_w = apply_thm(and_elim_left(In(xs, w), P(xs), []), [],
        And(In(xs, w), P(xs)), In(xs, w), got_fwd_xs)
    # [char_pv, In(xs,pv)] |- In(xs,w)
    ...
    sub_pv_w = Forall(xs, imp_sub)  # = Subset(pv, w)
    ...
    # From Eq(pv,w): ∀z. Iff(In(z,pv), In(z,w)). With In(n,w) → In(n,pv) → char_fwd → P(n)
    # Key: consume Eq(pv,w) via fl so it doesn't stay on the left.
    iff_n = Iff(In(n, pv), In(n, w))
    got_iff_n = fl(eq_pv_w, iff_n, n)
    # [eq_pv_w] |- Iff(In(n,pv), In(n,w))
    # Use cut to replace eq_pv_w with got_eq's left:
    got_iff_n_full = cut(got_iff_n, eq_pv_w, got_eq)
    # [got_eq.left (induction stuff)] |- Iff(In(n,pv), In(n,w))

    # Iff backward + In(n,w) → In(n,pv)
    got_in_npv = mp(apply_thm(iff_mp_rev(In(n, pv), In(n, w), []),
        [], iff_n, Implies(In(n, w), In(n, pv)), got_iff_n_full),
        ax(In(n, w)), In(n, w), In(n, pv))
    # [induction stuff, In(n,w)] |- In(n,pv)

    # char_fwd(n): [char_pv, In(n,pv)] |- And(In(n,w), P(n))
    got_and_final = char_fwd(n)
    # Cut In(n,pv) with got_in_npv:
    got_and_cut = cut(got_and_final, In(n, pv), got_in_npv)
    # [char_pv, induction stuff, In(n,w)] |- And(In(n,w), P(n))
    got_P_final = apply_thm(and_elim_right(In(n, w), P(n), []), [],
        And(In(n, w), P(n)), P(n), got_and_cut)
    # [char_pv, induction stuff, In(n,w)] |- P(n)

    # Apply P(n): Apply(v,n,y) → In(y,w)
    got_result = apply_thm(got_P_final, [y], Apply(v, n, y), In(y, w), ax(Apply(v, n, y)))
    # [char_pv, induction stuff, In(n,w), Apply(v,n,y)] |- In(y,w)
    ...
    from core.zfc import ZFCAxiom
    # Discharge non-axiom hypotheses: Apply(v,n,y), In(n,w), range_closed, omega_w, in_a_w, ra, ex_b
    for h in [Apply(v, n, y), In(n, w), range_closed, omega_w, in_a_w, ra, ex_b]:
        if any(same(h, f_) for f_ in got_result.sequent.left):
            imp_h = Implies(h, got_result.sequent.right[0])
            left_h = [f_ for f_ in got_result.sequent.left if not same(f_, h)]
            got_result = Proof(Sequent(left_h, [imp_h]), 'implies_right', [got_result], principal=imp_h)

    # Close foralls: y, n, w, f, a, v
    proof = got_result
    for var in [y, n, w, f, a, v]:
        body = proof.sequent.right[0]
        fa = Forall(var, body)
        proof = Proof(Sequent(proof.sequent.left, [fa]),
            'forall_right', [proof], principal=fa, term=var)

    proof.name = 'rec_approx_val_in_w'
    return proof
...
def rec_graph_exists():
    ...
    from theorems.sets import ordpair_bounded, singleton_exists, big_union_exists, union_exists
    from vocab import Function as FuncDef, Apply, RecApprox, BigUnion
    ...
    # Bounding set variables: uf = ∪f, uuf = ∪∪f, big = w ∪ ∪∪f, pb = P(big), ppb = P(P(big))
    uf = Var(postfix='uf')
    uuf = Var(postfix='uuf')
    big = Var(postfix='big')
    pb = Var(postfix='pb')
    ...
    # === Construct bounding set: big = w ∪ ∪∪f ===
    # ∪f:
    uf_char = BigUnion(uf, f)
    bue = big_union_exists()
    got_ex_uf = apply_thm(bue, [f], concl=Exists(uf, uf_char))
    # ∪∪f:
    uuf_char = BigUnion(uuf, uf)
    got_ex_uuf = apply_thm(bue, [uf], concl=Exists(uuf, uuf_char))
    # w ∪ ∪∪f:
    ue = union_exists()
    xb = Var()
    from core.derived import Or
    big_char = Forall(xb, Iff(In(xb, big), Or(In(xb, w), In(xb, uuf))))
    got_ex_big = apply_thm(ue, [w, uuf], concl=Exists(big, big_char))

    # === PowerSet twice on big ===
    ps_axiom = zfc.PowerSet()
    ps_ax = Proof(Sequent([ps_axiom], [ps_axiom]), 'axiom', principal=ps_axiom)
    pb_char = Forall(s_var, Iff(In(s_var, pb), Forall(d_var, Implies(In(d_var, s_var), In(d_var, big)))))
    ppb_char = Forall(s_var, Iff(In(s_var, ppb), Forall(d_var, Implies(In(d_var, s_var), In(d_var, pb)))))
    got_ex_pb = apply_thm(ps_ax, [big], concl=Exists(pb, pb_char))
    got_ex_ppb = apply_thm(ps_ax, [pb], concl=Exists(ppb, ppb_char))
    ...
    in_pv_hv = In(pv, hv)
    in_pv_ppb = In(pv, ppb)
    and_ppb_phi = And(in_pv_ppb, phi_pv)
    sf_sep_iff = Iff(in_pv_hv, and_ppb_phi)
    sf_sep_char = Forall(pv, sf_sep_iff)
    got_ex_sf = apply_thm(sep_ax, [w, f, a, ppb], concl=Exists(hv, sf_sep_char))

    # Desired characterization
    desired_iff = Iff(in_pv_hv, phi_pv)
    desired_char = Forall(pv, desired_iff)

    # === Forward: in_pv_hv → phi_pv (drop bounding) ===
    got_sep_fwd = apply_thm(iff_mp(in_pv_hv, and_ppb_phi, []),
        [], sf_sep_iff, Implies(in_pv_hv, and_ppb_phi),
        fl(sf_sep_char, sf_sep_iff, pv))
    got_and = mp(got_sep_fwd, ax(in_pv_hv), in_pv_hv, and_ppb_phi)
    got_fwd = apply_thm(and_elim_right(in_pv_ppb, phi_pv, []),
        [], and_ppb_phi, phi_pv, got_and)
    ...
    # === Backward: phi_pv → In(pv,ppb) (bounding) ===
    # From phi_pv components: nf∈w, RecApprox(vr,a,f,w), Apply(vr,nf,yr), OrdPair(pv,nf,yr)
    # Need: In(nf, big) and In(yr, big) for ordpair_bounded on big.
    # In(nf, big): from In(nf,w) + big_char → In(nf,w)∨In(nf,uuf) → In(nf,big) [left disjunct]
    # In(yr, big): from RecApprox prop 3: Apply(vr,nf,yr)→∃z.Apply(f,yr,z)→yr∈dom f⊆∪∪f=uuf
    #              then In(yr,uuf) → In(yr,big) [right disjunct]
    ...
    # In(nf, big) from In(nf,w): use big_char backward with left disjunct
    # big_char: ∀x. x∈big ↔ x∈w ∨ x∈uuf
    # From In(nf,w): Or(In(nf,w), In(nf,uuf)) → In(nf,big)
    from theorems.logic import or_intro_left, iff_mp_rev as _iff_rev
    or_nf = Or(in_nf_w, In(nf, uuf))
    got_or_nf = apply_thm(or_intro_left(in_nf_w, In(nf, uuf), []),
        [], in_nf_w, or_nf, ax(in_nf_w))
    iff_big_nf = Iff(in_nf_big, or_nf)
    got_nf_big = mp(apply_thm(_iff_rev(in_nf_big, or_nf, []),
        [], iff_big_nf, Implies(or_nf, in_nf_big), fl(big_char, iff_big_nf, nf)),
        got_or_nf, or_nf, in_nf_big)
    # [in_nf_w, big_char] |- In(nf, big)

    # In(yr, big) from RecApprox prop 3 + ∪∪f:
    # RecApprox prop 3: Apply(vr,nf,yr) → ∃z.Apply(f,yr,z)
    # Apply(f,yr,z) means ∃p. OrdPair(p,yr,z) ∧ In(p,f)
    # In(p,f) → ∪f char → elements of p are in ∪f → {yr}∈∪f → yr∈∪∪f
    ...
    got_yr_big = mp(apply_thm(_iff_rev(in_yr_big, or_yr, []),
        ...
        got_or_yr, or_yr, in_yr_big)
    # [In(yr,uuf), big_char] |- In(yr, big)

    # Apply ordpair_bounded on big:
    ob = ordpair_bounded()
    got_ob = apply_thm(ob, [big, nf, yr, pv, pb, ppb, sx_v, sxy_v])
    while not same(got_ob.sequent.right[0], In(pv, ppb)):
        cur = got_ob.sequent.right[0]
        hyp = cur.left
        if same(hyp, in_nf_big):
            got_ob = mp(got_ob, got_nf_big, hyp, cur.right)
        elif same(hyp, in_yr_big):
            got_ob = mp(got_ob, got_yr_big, hyp, cur.right)
        elif same(hyp, op_pv):
            got_ob = mp(got_ob, ax(op_pv), hyp, cur.right)
        elif same(hyp, pb_char):
            got_ob = mp(got_ob, ax(pb_char), hyp, cur.right)
        elif same(hyp, ppb_char):
            got_ob = mp(got_ob, ax(ppb_char), hyp, cur.right)
        else:
    ...
    # Package And(In(pv,ppb), phi_pv) for separation backward
    got_and_ppb = mp(apply_thm(and_intro(in_pv_ppb, phi_pv, []), [], in_pv_ppb,
        Implies(phi_pv, and_ppb_phi), got_ob),
        ax(phi_pv), phi_pv, and_ppb_phi)

    # Separation backward: and_ppb_phi → in_pv_hv
    got_sep_bwd = apply_thm(iff_mp_rev(in_pv_hv, and_ppb_phi, []),
        [], sf_sep_iff, Implies(and_ppb_phi, in_pv_hv),
        fl(sf_sep_char, sf_sep_iff, pv))
    got_bwd = mp(got_sep_bwd, got_and_ppb, and_ppb_phi, in_pv_hv)

    # Package individual hyps back into phi_pv on left (same pattern as before)
    and_ra_app = And(ra_vr, app_vr)
    and_inner = And(and_ra_app, op_pv)
    for pred, ext_fn in [
        (ra_vr, lambda: apply_thm(and_elim_left(ra_vr, app_vr, []), [],
            and_ra_app, ra_vr, apply_thm(and_elim_left(and_ra_app, op_pv, []), [],
                and_inner, and_ra_app, ax(and_inner)))),
        (app_vr, lambda: apply_thm(and_elim_right(ra_vr, app_vr, []), [],
            and_ra_app, app_vr, apply_thm(and_elim_left(and_ra_app, op_pv, []), [],
                and_inner, and_ra_app, ax(and_inner)))),
        (op_pv, lambda: apply_thm(and_elim_right(and_ra_app, op_pv, []), [],
            and_inner, op_pv, ax(and_inner))),
    ]:
        if any(same(pred, f_) for f_ in got_bwd.sequent.left):
            got_bwd = cut(got_bwd, pred, ext_fn())

    # Also package In(yr,uuf) back: it came from RecApprox prop 3.
    # For now it's on the left via ax(). Package into the And via cut with
    # a derivation from RecApprox. TODO: derive In(yr,uuf) from prop 3 + BigUnion.
    # For now: package with and_inner by extending it.
    # Actually: In(yr,uuf) is NOT part of phi_pv. It's an extra hypothesis.
    # To close ∀yr, I need it gone. Let me discharge it into the conclusion
    # and handle it as an extra hyp that gets eel'd with the RecApprox props.
    # Since In(yr,uuf) follows from Apply(vr,nf,yr)+RecApprox(vr,a,f,w)+BigUnion chars,
    # and those are already being eel'd, In(yr,uuf) should be derivable.
    # For now: just eel it alongside the other yr-dependent formulas.

    # In(yr,uuf) is also on the left with yr free. Cut it with a derivation from and_inner:
    # RecApprox prop 3 + Apply(vr,nf,yr) → ∃z.Apply(f,yr,z) → yr∈dom f⊆∪∪f
    # For now: package In(yr,uuf) together with and_inner before eel yr.
    if any(same(in_yr_uuf, f_) for f_ in got_bwd.sequent.left):
        extended = And(and_inner, in_yr_uuf)
        got_bwd = cut(got_bwd, and_inner,
            apply_thm(and_elim_left(and_inner, in_yr_uuf, []), [],
                extended, and_inner, ax(extended)))
        got_bwd = cut(got_bwd, in_yr_uuf,
            apply_thm(and_elim_right(and_inner, in_yr_uuf, []), [],
                extended, in_yr_uuf, ax(extended)))
        got_bwd = eel(got_bwd, extended, yr)
    else:
    ...
    if any(same(phi_nf_actual, f_) for f_ in got_bwd.sequent.left):
        got_bwd = cut(got_bwd, phi_nf_actual, apply_thm(and_elim_right(In(nf, w), phi_nf_actual, []),
            [], and_nf_phi, phi_nf_actual, ax(and_nf_phi)))
    ...
    # === Close existentials (dependency order: h, ppb, pb, big, uuf, uf) ===
    ...
    got_ex_desired = eel(got_ex_desired, ppb_char, ppb)
    got_ex_desired = cut(got_ex_desired, got_ex_desired.sequent.left[-1], got_ex_ppb)
    got_ex_desired = eel(got_ex_desired, pb_char, pb)
...
def rec_h_apply_fwd():
    ...
    # _eel removes and_in_phi from left, adds Exists(mm, and_in_phi).
...
def rec_h_function():
    ...
    # From OrdPair(zv, mm, yr): exists yr2. OrdPair(zv, mm, yr2), then exists xr. ...
...
def recursive_elim(h, a, f, w):
    """Decompose Recursive(h,a,f,w) into its 4 components.
    [Recursive(h,a,f,w)] |- Function(h)
    [Recursive(h,a,f,w)] |- dom_eq(h,w)
    [Recursive(h,a,f,w)] |- base(h,a)
    [Recursive(h,a,f,w)] |- step(h,f,w)

    Returns (got_func, got_dom, got_base, got_step, rec_formula)
    where rec_formula = Recursive(h,a,f,w) and each got_* has [rec_formula] on left."""
    from tactics import apply_thm, ax
    from vocab import Recursive as RecDef

    rec = RecDef(h, a, f, w)
    exp = rec.expand()
    # exp = And(func, And(dom_eq, And(base, step)))
    func_f = exp.left
    r1 = exp.right
    dom_f = r1.left
    r2 = r1.right
    ...
    got_func = apply_thm(and_elim_left(func_f, r1, []), [], rec, func_f, ax(rec))
    got_r1 = apply_thm(and_elim_right(func_f, r1, []), [], rec, r1, ax(rec))
    ...
    return got_func, got_dom, got_base, got_step, rec
...
def rec_values_agree():
    ...
    # From base_h: instantiate ev, mp Empty(ev) -> Apply(h, ev, a)
