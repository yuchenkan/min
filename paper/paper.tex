...
language of scientific textbooks. The proofs are written in a custom DSL
...
custom DSL (\texttt{.min} files). It reads theorem statements and existing
...
Proof language & custom DSL (.min files) \\
...
DSL, and the entire proof library was ported (blue curve). The DSL rewrite
...
\caption{Total lines of proof code over the development period. Black:
  Python prototype; blue: rewrite to C kernel with custom DSL. Both
