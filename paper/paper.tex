...
\paragraph{Division of roles.}
The restriction to Zermelo set theory (Z) illustrates how the three
components interact. The LLM's initial proof attempts used Replacement and
Regularity freely; the human architect imposed the restriction to Z,
requiring the LLM to find alternative proofs. This required non-trivial
substitutes: successor injectivity was reproved via transitive sets
(Lemma~4.2.4 of the textbook) rather than Regularity, and the recursion
theorem's graph construction used power set bounding
($\mathcal{P}(\mathcal{P}(\omega \cup \bigcup\bigcup f))$) rather than
Replacement. The kernel verified that no Replacement, Regularity, or Choice
appears in the axiom dependencies --- all 199 theorems use only Z.
